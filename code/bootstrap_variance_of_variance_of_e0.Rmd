...
combined_data = NULL
combined_data[[1]] = us_data
combined_data = c(combined_data, state_data)
```

Mx values
```{r}
Mx = lapply(combined_data, function(x) {
  filtered_df <- x[x$Year == 1989, ]
  
  return(filtered_df$mx)
})

state_names <- sapply(combined_data, function(df) as.character(df$PopName[1]))
...
N = subset(pop_89, !(rownames(pop_89) %in% c("DC")))
...
# save as csv
write.csv(df, paste(path, "boot_results_v2.csv", sep = "/"), row.names = TRUE)
```
